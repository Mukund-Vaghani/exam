...
var user = {
    ...
    getSavedCards: function (req, callback) {
        var sql = `SELECT c.id,tc.card_name,c.card_holder_name,c.card_number,c.expiry_date FROM tbl_card c JOIN tbl_card_type tc ON tc.id=c.card_type_id WHERE c.user_id = ? AND c.is_active = '1' AND c.is_delete = '0'`
        con.query(sql, [req.user_id], function (error, result) {
